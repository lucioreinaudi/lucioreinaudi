...
def mostrarInscripciones(conexion):
    """Muestra los grupos ya sorteados si existieran, usando la conexión de tu compañera."""
    ...
    print("\n" + "=" * 80)
    print(f"{'NOMBRE COMPLETO':<25} | {'TRIBU Y GÉNERO':^18} | {'ACTIVIDAD INSCRIPTO':<30}")
    print("=" * 80)
    ...
    for fila in resultados:
        ...
        print(f"{nombre_completo:<25} | {tribu_genero:^18} | {fila[4]:<30}")
    
    print("=" * 80)
    cursor.close()

def tablaIinscripcionesVacia():
    """Verifica si la tabla de inscripciones está vacía. Si no lo está, muestra los datos."""
    ...
    try:
        ...
        if cantidad == 0:
            print("La tabla 'inscripciones' está vacía. Se puede iniciar el sorteo.")
            return True
        else:
            print(f"La tabla ya tiene {cantidad} inscripciones. Mostrando los grupos existentes:")
            mostrarInscripciones(conexion) 
            return False
...
def ejecutar_opcion(opcion):
    ...
    if opcion == '1':
        conn = conectar_bd()
        if conn:
            print(f"\nLa tabla ya tiene {tablaIinscripcionesVacia()} inscripciones. Mostrando los grupos existentes:")
        ...
        if df_inscripciones.empty:
            print("No se encontraron inscripciones. La tabla está vacía o hay un error de lectura.")
            return
        ...
        mostrarResultados(resultados, actividades_disponibles, cupos)
